#!/usr/bin/env python3
"""
OpenUxAS Korean Guide - Markdown to PDF converter
Uses fpdf2 with WenQuanYi Zen Hei font for Korean text support.
Handles: headings, tables, code blocks, mermaid diagrams (as text),
...
INPUT_FILE = "OpenUxAS_Guide_Korean.md"
OUTPUT_FILE = "OpenUxAS_Guide_Korean.pdf"
...
class GuidePDF(FPDF):
    ...
    def header(self):
        if self.page_no() > 1:
            ...
            self.cell(0, 8, "OpenUxAS 초보자를 위한 완벽 가이드", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    def write_rich_text(self, text, size=10, line_height=6, align="L"):
        """Write text with **bold** and *italic* inline formatting."""
    ...
    def add_heading(self, level, text):
        ...
        if level == 1:
            ...
            self.set_font("Main", "B", size)
            self.ln(4)
            self.cell(CONTENT_W, 14, f"  {text}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            self.set_text_color(0, 0, 0)
            self.ln(4)
        elif level == 2:
            self.set_text_color(25, 60, 120)
            self.set_font("Main", "B", size)
            self.cell(CONTENT_W, 10, text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            ...
            self.set_font("Main", "B", size)
            self.cell(CONTENT_W, 9, text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            self.set_text_color(0, 0, 0)
            self.ln(2)
        else:
            self.set_font("Main", "B", size)
            self.cell(CONTENT_W, 8, text, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            self.ln(2)
    ...
    def add_code_block(self, lines, language=""):
        ...
        if language and language != "mermaid":
            ...
            self.cell(block_w, 4, f"  {language}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            self.set_text_color(0, 0, 0)
        ...
        for line in lines:
            ...
            self.cell(block_w, 4, f"  {display_line}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    def add_table(self, headers, rows):
        ...
        # Adjust column widths - give more space to wider content
    ...
    def add_blockquote(self, text):
        """Add a blockquote with left border."""
...
def parse_and_render(pdf, md_text):
    ...
    pdf.cell(CONTENT_W, 12, "초보자를 위한 완벽 가이드", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(5)
    pdf.set_font("Main", "", 14)
    pdf.set_text_color(80, 80, 80)
    pdf.cell(CONTENT_W, 10, "Complete Guide for Beginners", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(8)
    pdf.set_font("Main", "", 11)
    pdf.cell(CONTENT_W, 8, "다수 무인 비행체 자율 시스템 프레임워크", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.cell(CONTENT_W, 8, "Multi-Vehicle Autonomous Systems Framework", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(15)
    ...
    pdf.cell(CONTENT_W, 8, "이론 | 설치 | 아키텍처 | 예제 실습 | 결과 분석", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(10)
    pdf.cell(CONTENT_W, 8, "Air Force Research Laboratory (AFRL)", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.cell(CONTENT_W, 8, "Aerospace Systems Directorate", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    pdf.ln(20)

    pdf.set_font("Main", "I", 9)
    pdf.set_text_color(120, 120, 120)
    pdf.cell(CONTENT_W, 8, "14개 Mermaid 시퀀스 다이어그램 포함", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align="C")
    ...
    pdf.set_font("Main", "B", 18)
    pdf.set_text_color(25, 60, 120)
    pdf.cell(CONTENT_W, 12, "목차 (Table of Contents)", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.set_draw_color(25, 60, 120)
    ...
    # Skip the frontmatter lines (title, subtitle, etc.) - start from first ---
    ...
    while i < len(lines):
        ...
        if line.strip().startswith("```"):
            if in_code_block:
                ...
                if is_mermaid:
                    # For mermaid, render as a special labeled code block
                    pdf.set_font("Main", "B", 9)
                    pdf.set_text_color(100, 50, 150)
                    pdf.cell(CONTENT_W, 6, "[Diagram] Mermaid Sequence Diagram (렌더링은 GitHub/Mermaid 뷰어에서 확인)",
        ...
        # Skip frontmatter until we hit first chapter heading
        if skip_frontmatter:
        ...
        if stripped.startswith("#"):
            ...
            # Remove inline formatting for headings
            text = text.replace("*", "")

            # New page for chapter headings (level 1)
            if level == 1:
                pdf.add_page()
